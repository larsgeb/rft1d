...
def rft(c, k, STAT, Z, df, R, n=1, Q=None, expectations_only=False, version="spm12"):
    ...
    if version == "spm8":
        ...
        raise (
            ValueError(
                'rft1d error:  unknown version "%s" (version must be "spm8" or "spm12")'
                % str(version)
            )
        )
    if (
        n == 1
    ):  # take a shortcut (Edit TCP 2014.08.11) -- about 9 times faster than the fast version below
        EM = R * EC
        EN = EC[0] * R[-1]
    else:
    ...
    if (
        version == "spm8"
    ):  # non-implemented flags are removed in spm12; rft1d validates all cases (see ./rft1d/examples/val*)
        if STAT in ["T", "X2"]:
            if (k > 0) and (n > 1):
                P, p = None, None
        elif STAT == "F":
...
def isf(STAT, alpha, df, resels, n, Q=None, version="spm12"):
    ...
    for aaa in alpha:
        ...
        fn = (
            lambda x: (rft(1, 0, STAT, x[0], df, resels, n, Q, False, version)[0] - aaa)
            ** 2
        )
...
class _Expected(object):
    ...
    def number_of_suprathreshold_nodes(self, u):
        ...
        """
        return self._calc.FWHM * self.number_of_suprathreshold_resels(
            u
        ) + self.number_of_upcrossings(u)
...
class RFTCalculator(object):
    ...
    def _parse_nodes_argument(self, nodes):
        if isinstance(nodes, int):
            ...
            if nodes.ndim != 1:
                raise (
                    ValueError(
                        'RFT1D Error:  the "nodes" argument must be a 1D boolean array. Received a %dD array'
                        % arg.ndim
                    )
                )
            ...
            raise (
                ValueError(
                    'RFT1D Error:  the "nodes" argument must be an integer or a 1D boolean array'
                )
            )
    ...
    def set_fwhm(self, w):
        ...
        if self.mask is None:
            self.resels = (
                1,
                (self.nNodes - 1) / self.FWHM,
            )  # field length is (nNodes - 1)
        else:
...
class RFTCalculatorResels(RFTCalculator):
    ...
    def set_bonf(self, wBonf):
        ...
        if self.withBonf and (self.nNodes is None):
            raise (
                ValueError(
                    'Must specify an integer value for "nNodes" when "withBonf" is True.'
                )
            )
        self.Q = float(self.nNodes) if self.withBonf else None
