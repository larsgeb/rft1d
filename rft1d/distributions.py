                ...
                for parent in cls.__bases__:
                    ...
                    if parfunc and getattr(parfunc, "__doc__", None):
                        ...
                        if ndf == 0:
                            ...
                            s = s.replace(
                                "degrees of freedom (int or float)",
                                "degrees of freedom (two-tuple of int or float)",
                            )
                        func.__doc__ = s
                        break
        return cls
...
class _RFTDistribution(object):
    ...
    def isf(self, alpha, df, nodes, FWHM, withBonf=False):
        ...
        df = self._get_df(df)
        E = RFTCalculator(
            STAT=self._STAT, df=df, nodes=nodes, FWHM=FWHM, withBonf=withBonf
        )
        return E.isf(alpha)
    ...
    def isf_resels(self, alpha, df, resels, withBonf=False, nNodes=None):
        ...
        """
        df = self._get_df(df)
        E = RFTCalculatorResels(
            STAT=self._STAT, df=df, resels=resels, withBonf=withBonf, nNodes=nNodes
        )
        return E.isf(alpha)
